...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
        ...
        if (!$this->config['disabled']) {
            ...
            if (!is_dir($this->typeDir)) {
                if (!mkdir($this->typeDir, $this->config['dirPerms'])) {
                    $this->backMsg("Cannot create {dir} folder. ({file})", array('dir' => $this->type,'file'=>$this->typeDir));
                }
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            if (isset($_GET['dir']) &&
                ...
                if (substr($udir, 0, strlen($dir)) !== $dir)
                    $message = $this->label("Unknown error. 3");
                else {
                    $l = strlen($dir);
                    $dir = "$udir/";
                    $udir = substr($udir, $l);
    ...
    protected function checkFilePath($file) {
        ...
        //hack Air Santé... >> on supprime tout ce qui se trouve avant kcfinder dans les deux var à tester
        $path1 = substr($rPath, strripos($rPath, "kcfinder"));
        $typeDir = substr($this->typeDir, strripos($this->typeDir, "kcfinder"));
        return $path1 === $typeDir;
        
        //return (substr($rPath, strlen($rPath)-strlen($this->typeDir)) === $this->typeDir);  >> v2
        //return (substr($rPath, 0, strlen($this->typeDir)) === $this->typeDir);	>> v originale
    }
    ...
    protected function checkUploadedFile(array $aFile=null) {
        ...
        if (!is_array($file) || !isset($file['name']))
            return $this->label("Unknown error 1");
