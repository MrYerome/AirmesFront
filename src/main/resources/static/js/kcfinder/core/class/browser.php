...
class browser extends uploader {
    ...
    protected function getFiles($dir) {
        ...
        // $thumbDir = upload/.thumbs/articles/ -- $dir = upload/articles/
        $return = array();
    ...
    protected function postDir($existent=true) {
        ...
        if (!$this->checkFilePath($dir)) {
            //$this->errorMsg("Unknown error. 18 ".$this->typeDir . " -- ".substr(realpath($dir), 0, strlen($this->typeDir)));
            $this->errorMsg("Unknown error. 18 ".substr($this->typeDir, strripos($this->typeDir, "kcfinder")) . " -- ".substr(realpath($dir), strripos(realpath($dir), "kcfinder")));
            //;
        }
    ...
    protected function getDir($existent=true) {
        ...
        if (isset($_GET['dir']))
            $dir .= "/" . $_GET['dir'];
        if (!$this->checkFilePath($dir))
            $this->errorMsg("Unknown error. 19");
        if ($existent && (!is_dir($dir) || !is_readable($dir)))
            $this->errorMsg("Inexistant or inaccessible folder.");
        return $dir;
    }
